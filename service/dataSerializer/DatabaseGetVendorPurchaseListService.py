...
class DatabaseGetVendorPurchaseListService(IDataSerializer):
    def __init__(self, bg, site, plant, startdate, enddate, vendor_type, reimburse_type, vendor_name, pageSize, pageNo):
        self.bg = bg
        self.site = site
        self.plant = plant
    ...
    def serialize(self):
        ...
        """
        # todo dao count amount po_number (po_price_in_ntd po_amount_in_ntd 尚為空)
        try:
            ft = []
            if bg[0] != '':
            ...
            ft.append(ObjPurchaseOrder.vendor_name == vendor_name)

            results = db.session.query(func.sum(ObjPurchaseOrder.po_amount).label('amount'),
                                       ObjPurchaseOrder.mmp_bg.label('bg'),
                                       ObjPurchaseOrder.mmp_site.label('site'),
                                       ObjPurchaseOrder.charge_plant_code.label('plant')) \
                .filter(*ft).group_by(ObjPurchaseOrder.mmp_bg, ObjPurchaseOrder.mmp_site,
                                      ObjPurchaseOrder.charge_plant_code)\
                .order_by(func.sum(ObjPurchaseOrder.po_amount))
