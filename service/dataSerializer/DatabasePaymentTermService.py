...
class DatabasePaymentTermService(IDataSerializer):
    ...
    def serialize(self):
        ...
        logger.debug("------DatabasePaymentTermService Param: ")
        ...
        if bg == '' and site == '' and plant == '':
            queryTyoe = 1
        if queryTyoe == 1:
            sqlquery = 'select po.payment_term as paym, poc.mmp_charge, sum(po.po_amount) as amount ' \
                       'from public."PURCHASE_ORDER" po ' \
                       'inner join public."PURCHASE_ORDER_CHARGE" poc ' \
                       'on po.po_no = poc.po_no ' \
                       'group by payment_term ,poc.mmp_charge ' \
                       'order by sum(po.po_amount), poc.mmp_charge '
        else:
            sqlquery = 'select po.payment_term as paym, poc.mmp_charge, sum(po.po_amount) as amount ' \
                       'from public."PURCHASE_ORDER" po ' \
                       'inner join public."PURCHASE_ORDER_CHARGE" poc ' \
            ...
            sqlquery += 'group by payment_term ,poc.mmp_charge order by sum(po.po_amount), poc.mmp_charge '
        print(sqlquery)
